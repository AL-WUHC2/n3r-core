...
public class MathArithmeticFactory implements WordFactory {
    ...
    public static String rand(int n) {
        ...
        char c1 = s1.charAt(RRand.randInt(s1.length()));
        return "" + c1;
    }

    @Override
    public WordBean getNextWord() {
        int n1 = RRand.randInt(9) + 1;
        int n2 = RRand.randInt(9) + 1;
        ...
        int op = RRand.randInt(4);
        String opStr = operations[op];
        char opCh = opStr.charAt(RRand.randInt(opStr.length()));

        int expected = 0;
        String result = "";
        int askPos = RRand.randInt(3);
        if (op == 0) { // + 
            if (askPos == 0) {
