...
public class SymbolDiffFactory implements WordFactory {
    ...
    public WordBean getNextWord() {
        int randPos1 = RRand.randInt(symbols.length);
        int randPos2 = randPos1;
        while (randPos2 == randPos1) {
            randPos2 = RRand.randInt(symbols.length);
        }
        String group1 = symbols[randPos1];
        String group2 = symbols[randPos2];
        char ch1 = group1.charAt(RRand.randInt(group1.length()));
        char ch2 = group2.charAt(RRand.randInt(group2.length()));

        int len = 4 + RRand.randInt(2);
        int diffPos = RRand.randInt(len);
        String word = RStr.repeat("" + ch1, diffPos) + ch2 + RStr.repeat("" + ch1, len - diffPos - 1) ;
