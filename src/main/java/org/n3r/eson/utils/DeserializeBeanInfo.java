...
import org.n3r.eson.annotation.EsonCreator;
...
public class DeserializeBeanInfo {
    ...
    public FieldInfo getField(String propertyName) {
        for (FieldInfo item : fieldList)
            if (item.getName().equals(propertyName)) return item;
    ...
    public boolean add(FieldInfo field) {
        for (FieldInfo item : fieldList)
            if (item.getName().equals(field.getName())) return false;
        fieldList.add(field);
    ...
    public static DeserializeBeanInfo computeSetters(Class<?> clazz, Type type) {
        ...
        if (defaultConstructor != null) {
            ...
            if (creatorConstructor != null) {
                ...
                for (int i = 0; i < creatorConstructor.getParameterTypes().length; ++i) {
                    ...
                    EsonField esonField = null;
                    for (Annotation paramAnnotation : paramAnnotations)
                        if (paramAnnotation instanceof EsonField) {
                            esonField = (EsonField) paramAnnotation;
                            break;
                        }
                    if (esonField == null) throw new EsonException("illegal json creator");

                    Class<?> fieldClass = creatorConstructor.getParameterTypes()[i];
                    Type fieldType = creatorConstructor.getGenericParameterTypes()[i];
                    Field field = getField(clazz, esonField.name());
                    if (field != null) field.setAccessible(true);
                    FieldInfo fieldInfo = new FieldInfo(esonField.name(), clazz, fieldClass, fieldType, null,
                            field);
            ...
            if (factoryMethod != null) {
                ...
                for (int i = 0; i < factoryMethod.getParameterTypes().length; ++i) {
                    ...
                    EsonField fieldAnnotation = null;
                    for (Annotation paramAnnotation : paramAnnotations)
                        if (paramAnnotation instanceof EsonField) {
                            fieldAnnotation = (EsonField) paramAnnotation;
                            break;
                        }
                    if (fieldAnnotation == null) throw new EsonException("illegal json creator");
            ...
            ObjectInstantiator objectInstantiator = new ObjenesisStd().getInstantiatorOf(clazz);
            beanInfo.setObjectInstantiator(objectInstantiator);
            // throw new EsonException("default constructor not found. " + clazz);
        }
        ...
        for (Method method : clazz.getMethods()) {
            ...
            // support builder set
            if (!(method.getReturnType().equals(Void.TYPE) || method.getReturnType().equals(clazz))) continue;
    ...
    public static Constructor<?> getCreatorConstructor(Class<?> clazz) {
        ...
        for (Constructor<?> constructor : clazz.getDeclaredConstructors()) {
            EsonCreator annotation = constructor.getAnnotation(EsonCreator.class);
            if (annotation != null) {
                if (creatorConstructor != null) throw new EsonException("multi-json creator");
    ...
    public static Method getFactoryMethod(Class<?> clazz) {
        ...
        for (Method method : clazz.getDeclaredMethods()) {
            ...
            EsonCreator annotation = method.getAnnotation(EsonCreator.class);
            if (annotation != null) {
                if (factoryMethod != null) throw new EsonException("multi-json creator");
