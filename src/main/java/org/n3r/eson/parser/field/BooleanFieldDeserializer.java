...
public class BooleanFieldDeserializer extends FieldDeserializer {
    ...
    public BooleanFieldDeserializer(ParserMapping mapping, Class<?> clazz, FieldInfo fieldInfo) {
    ...
    public void parseField(JSONParser parser, Object object, Type objectType, Map<String, Object> fieldValues) {
        ...
        if (lexer.token() == JSONToken.TRUE) {
            ...
            if (object == null) fieldValues.put(fieldInfo.getName(), Boolean.TRUE);
            else setValue(object, true);
            return;
        }
        ...
        if (lexer.token() == JSONToken.LITERAL_INT) {
            ...
            if (object == null) fieldValues.put(fieldInfo.getName(), booleanValue);
            else setValue(object, booleanValue);
            return;
        }
        ...
        if (lexer.token() == JSONToken.NULL) {
            ...
            if (getFieldClass() == boolean.class) // skip
                return;
        ...
        if (lexer.token() == JSONToken.FALSE) {
            lexer.nextToken(JSONToken.COMMA);
            if (object == null) fieldValues.put(fieldInfo.getName(), Boolean.FALSE);
            else setValue(object, false);
            return;
        }
        ...
        if (value == null && getFieldClass() == boolean.class) // skip
            return;
